...
- [x] 1.1 Add a `speechSupported` check on mount
      (`'SpeechRecognition' in window || 'webkitSpeechRecognition' in window`).
- [x] 1.2 Add `isListening` and `voiceError` state, and a `source` ref/state
      (`'text' | 'voice'`, default `'text'`) to track submission provenance.

## 2. Microphone button & recognition wiring

- [x] 2.1 Render the microphone button next to the existing textarea, only when
      `speechSupported` is true.
- [x] 2.2 On click, instantiate `SpeechRecognition`/`webkitSpeechRecognition` with
      `continuous: false`, `interimResults: true`, and start listening; set
      `isListening` true.
- [x] 2.3 On `onresult`, write the interim/final transcript into the existing `text`
      state and set `source` to `'voice'`.
- [x] 2.4 On `onend`, set `isListening` false.
- [x] 2.5 On `onerror`, set a user-facing `voiceError` message, set `isListening`
      false, and ensure nothing is submitted.

## 3. Manual-edit provenance reset

- [x] 3.1 In the textarea's existing `onChange` handler, reset `source` to `'text'`
...
- [x] 4.1 In `handleSubmit`, tag newly created tasks with the current `source` value
      instead of the hardcoded `'text'`.
- [x] 4.2 Reset `source` back to `'text'` after a successful submit, alongside the
      existing `setText("")`.

## 5. UI feedback

- [x] 5.1 Show a listening indicator on the mic button while `isListening` is true
      (e.g. label/style change), reusing the existing loading-state visual pattern.
- [x] 5.2 Render `voiceError` using the same inline error style already used for
      parse errors.
